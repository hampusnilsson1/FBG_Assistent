...
def parse_sitemap(xml_data):
    ...
    for url in root.findall(".//sitemaps:url", ns):
        ...
        lastmod = (
            url.find("sitemaps:lastmod", ns).text
            if url.find("sitemaps:lastmod", ns) is not None
            else None
        )
        urls.append(loc)
...
def scrape_page(url, lastmod, driver, existing_urls):
    ...
    for link in pdf_links:
        ...
        save_to_json(
            pdf_data, existing_urls=existing_urls
        )  # Save each PDF data immediately
    return {"url": url, "last_modified": lastmod, "title": title, "texts": texts}
...
def save_to_json(
    ...
    if update_mode == "update_since":
        if data["url"] in existing_urls:
            existing_lastmod = datetime.strptime(
                existing_urls[data["url"]]["last_modified"], "%Y-%m-%dT%H:%M:%SZ"
            )
