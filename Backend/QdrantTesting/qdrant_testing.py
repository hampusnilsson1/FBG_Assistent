...
def search_partialmatch(url):
    qdrant_filter = models.Filter(
        should=[
            models.FieldCondition(key="metadata.url", match=models.MatchText(text=url)),
            models.FieldCondition(
                key="metadata.source_url", match=models.MatchText(text=url)
            ),
        ]
    )
...
def search_contentmatch(text):
    qdrant_filter = models.Filter(
        must=[models.FieldCondition(key="content", match=models.MatchText(text=text))]
    )
...
def print_result(result):
    for point in result:
        id = point.id
        chunk = point.payload.get("content", "")[:5000]
        chunk_info = point.payload.get("metadata.chunk_info", "")
        title = point.payload.get("metadata.title", "")
        url = point.payload.get("metadata.url", "")
        source_url = point.payload.get("metadata.source_url", "")
        update_date = point.payload.get("metadata.update_date", "")
        print(f"**{title}**")
        print(f"ID: {id}")
        print(f"Chunk: {chunk}")
...
while True:
    input_url = input(
        "1. Search for Exact URL/Source URL Match \n2. Search for datapoints including input\n3. Search for content match\n4. Exit Application\n"
    )
    print("-" * 50)
    if input_url == "1":
        ...
        print("Search for content match")
        text = str(input("Enter the text to search for: "))
        result = search_contentmatch(text)
